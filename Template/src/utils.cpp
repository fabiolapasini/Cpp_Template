...
void from_json(const json& j, utils::ImageInfo& mpi) {
    ...
    mpi.Width = static_cast<uint16_t>(std::stoi(j.at("Width").get<std::string>()));
    mpi.Height = static_cast<uint16_t>(std::stoi(j.at("Height").get<std::string>()));
    mpi.Channels = static_cast<uint16_t>(std::stoi(j.at("Channels").get<std::string>()));
}
