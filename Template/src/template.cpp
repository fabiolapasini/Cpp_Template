...
int runProgram(const utils::Configuration& config) {
    ...
    size_t bufferSize =
        utils::getTotalBytes(config.ImageInfo.Width, config.ImageInfo.Height,
                             config.ImageInfo.Channels);
    std::vector<uint8_t> buffer(bufferSize);
    file.read(reinterpret_cast<char*>(buffer.data()), bufferSize);
    file.close();

    cv::Mat image(config.ImageInfo.Height, config.ImageInfo.Width, CV_8UC3,
                  buffer.data());
    cv::Mat bgrImage;
